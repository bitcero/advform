...
class RMFormSlider extends RMFormElement
{
    ...
    public function __construct($caption, $name, $default = [])
    {
        ...
        if (is_array($caption)) {
            ...
            $this->setWithDefaults('default', $default, []);
        }
    ...
    public function render()
    {
        ...
        foreach ($this->get('default') as $index => $slider) {
            ...
            foreach ($slider as $id => $value) {
                if ('adv-field-types' == $id || 'adv-field-captions' == $id) {
                    continue;
                }
                ...
                switch ($types[$id]) {
                    ...
                    case 'imageurl':
                        ...
                        break;
                    case 'textbox':
                        $fields .= '<input type="text" class="form-control" name="' . $this->get('name') . '[' . $index . '][' . $id . ']"' . (0 == $ic ? ' class="the-title"' : '') . ' value="' . $tc->specialchars($value) . '">';
                        break;
                }
        ...
        $rtn = '<div ' . $attributes . '>';
        $rtn .= $fields;
        $rtn .= '<div class="data-fields">' . json_encode(['name' => $this->get('name'), 'fields' => $this->fields]) . '</div>';
